...
import 'package:mywebsite/constants/text_styles.dart';
import 'package:mywebsite/models/education.dart';
import 'package:mywebsite/widgets/circle_widget.dart';
...
//            left: MediaQuery.of(context).size.height * 0.10,
  ...
  Widget _buildWork({double? headingFontSize}) {
    return Column(
      mainAxisSize: MainAxisSize.min,
      crossAxisAlignment: CrossAxisAlignment.start,
      children: <Widget>[
        _buildWorkContainerHeading(fontSize: headingFontSize),
        SizedBox(height: 8.0),
        _buildWorkSummary(),
        SizedBox(height: 8.0),
        _buildWorkTimeline(headingFontSize: headingFontSize)
      ],
    );
  }

  Widget _buildWorkContainerHeading({double? fontSize}) {
    return Text(
      Strings.experience,
      style: TextStyles.body.copyWith(fontSize: fontSize),
    );
  ...
  Widget _buildWorkSummary() {
    return Text(
      Strings.work_summary,
      style: TextStyles.body.copyWith(color: Colors.white),
    );
  }

  Widget _buildWorkTimeline({double? headingFontSize}) {
    final widgets = Strings.educationList
        .map(
          (education) => _buildWorkTile(
            education,
  ...
  Widget _buildWorkTile(Education education, {double? headingFontSize}) {
    return Padding(
